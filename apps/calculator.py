...
def venmo_calc(
    ...
    if convert:
        (
            ...
            my_dic, total, tax, tip, misc_fees, contribution, discount
        )
    ########################################

    precheck_sum = round(
        sum(my_dic.values()) + tax + tip + misc_fees + discount + contribution, 2
    )
    ...
    if total != precheck_sum:
        ...
        store_perc = contribution / (
            total - tip - misc_fees - tax - discount - contribution
        )
        ...
        for key in my_dic.keys():
            ...
            person_total = (
                my_total + tax_part + fee_part + tip_part + disc_part + store_part
            )
...
2. All tax, tip, fees, totals were converted to USD. __1 USD = {usd_convert[0]}__.
    * The rate was last updated on {usd_convert[1]} using information provided by [RatesAPI](https://ratesapi.io/)
        ...
        if rounded_sum > total + 0.1:
            st.error(
                f"Uh oh! My calculated venmo charge sum is ${rounded_sum} but the receipt total was ${round(total,2)}, a difference of ${round(abs(rounded_sum-total),2)}"
            )
            st.stop()
        ...
        data = {"request_money": request_money, "messages": messages}
        return data


def venmo_message_maker(
    description,
    request,
    my_dic,
    tip_perc,
...
def total_calculator(
    ...
    raw_pairs = [
        (rf.parse_alpha(alpha), sum([float(i) for i in rf.parse_numbers(numbers)]))
    ...
    total_value = round(
        precheck_sum + tax_input + tip_input + fees_input + discount + contribution, 2
    )  # prefill the total
...
def html_table(link_output, request_money):
    ...
    link_type = st.selectbox(
        "Request payments yourself, or send payme links to your friends",
        options=["Request them", "Pay me"],
    )
    ...
    venmo_logo = (
        "https://raw.githubusercontent.com/pomkos/payme/main/images/venmo_logo_blue.png"
    )
