            ...
            try:
                ...
                if (num_meals >= 1) & (num_meals <= 9):
                    ...
                    for i, v in my_series.loc[loc1 : loc2 - 1].items():
                        extras_loc.append(
                            i
                        )  # get the indexes of all items between loc and loc2
...
def receipt_formatter(receipt, names_dict, names, promo=True):
    ...
    name_counter = (
        {}
    )  # to count the number of times something appears, it appears more than once then its a bug (tip appears in tip and tipo)
    for name in names:
        name_counter[name] = [0]
    ...
    for k, v in name_counter.items():
        if v[0] > 1:
            st.warning(
                f"It looks like {k} appears more than once in the receipt, at lines {name_counter[name][1:]}. Please find the correct {k} and rename the others. Then tell Pete!"
            )
            st.stop()
    ...
    for name, items in names_str_items.items():
        names_str_items[name] = items.apply(
            lambda x: "ADDON" if re.findall("^(\s){2,}", x) else x
        )
    # Check for extras
...
def app(receipt, my_names, description):
    ...
    if promo:
        st.info(
            "Note: for calculation purposes, promotions are divided equally and subtracted from each total"
        )
